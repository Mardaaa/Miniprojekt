...
def visualize_results(board_img, components, crown_coordinates, component_scores):
    # Define colors for different tile types for visualization
    color_map = {
        'Lake': (255, 0, 0),  # Blue
        'Field': (0, 255, 0),  # Green
        'Swamp': (0, 0, 255),  # Red
        'Grassland': (255, 255, 0),  # Cyan
        'Forrest': (0, 255, 255),  # Yellow
        'Home': (255, 0, 255)  # Magenta
    }

    # Draw each component with its respective color
    tile_height, tile_width, _ = board_img.shape
    tile_height //= 5  # Adjust based on grid size
    tile_width //= 5  # Adjust based on grid size
    ...
    for tile_type, blobs in components.items():
        color = color_map.get(tile_type, (127, 127, 127))  # Default to gray if no color specified

        for blob_index, blob in enumerate(blobs):
            for coord in blob:
                top_left = (coord[1] * tile_width, coord[0] * tile_height)
                bottom_right = ((coord[1] + 1) * tile_width, (coord[0] + 1) * tile_height)
                cv2.rectangle(board_img, top_left, bottom_right, color, -1)  # Fill the tile with color

            # Place text in the first tile of each blob
            first_tile = list(blob)[0]
            text_pos = (first_tile[1] * tile_width + 5, first_tile[0] * tile_height + 15)
            score_text = f"C:{component_scores[(tile_type, blob_index)]}"
            cv2.putText(board_img, score_text, text_pos, font, font_scale, font_color, font_thickness)

    crown_icon = cv2.imread('/Users/jens-jakobskotingerslev/Desktop/Miniprojekt semester 2/Scripts/Crown images/field_crown_normal.jpg')  # If you have an icon, otherwise use a simple marker
    for crown in crown_coordinates:
        center_pos = (crown[1] * tile_width + tile_width // 2, crown[0] * tile_height + tile_height // 2)
        if crown_icon is not None:
            # Adjust this if you're using an icon, ensure it's resized properly
            start_pos = (center_pos[0] - crown_icon.shape[1] // 2, center_pos[1] - crown_icon.shape[0] // 2)
            board_img[start_pos[1]:start_pos[1]+crown_icon.shape[0], start_pos[0]:start_pos[0]+crown_icon.shape[1]] = crown_icon
        else:
            cv2.circle(board_img, center_pos, 10, (0, 0, 0), -1)  # Draw a simple black circle for crowns
...
if __name__ == '__main__':
    ...
    crown_coordinates = TemplateMatching(image_files, template_files)
    _, component_scores = calculate_total_score(crown_coordinates, components)
    visualize_results(board_img.copy(), components, crown_coordinates, component_scores)
